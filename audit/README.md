...
- **A26** Missing focus indicator on interactive glass — CSS file with `.lg-glass` rules ships no `:focus-visible { outline: ... }` rule (WCAG 2.4.7). See `spec/rules/accessibility-rules.md`.
- **A27** Icon-only glass action missing accessible name — `<button>` / `<summary>` with class `lg-icon-button` / `lg-toolbar-pill__item` / `lg-floating-hud__item` / `lg-sidebar-toggle` / `lg-stepper__button` / `lg-toolbar-button` ships no `aria-label` / `aria-labelledby` / `title` (WCAG 4.1.2). See `spec/rules/accessibility-rules.md`.
...
- **A** — anti-patterns. A1–A10 are cross-cutting; A11–A24 (in the native skill only) are macOS 26-specific. A25 (renderer tier), A26 (focus indicator), and A27 (icon-only accessible name) are web-only.
